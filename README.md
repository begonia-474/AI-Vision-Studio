...
- **自添加模型**：无需写代码，以任意内置模型为模板（继承其尺寸机制与参数分区），替换模型 ID / 名称并覆盖默认参数，即可接入同厂商新模型（魔搭模型支持自由参数与 LoRA）；提交时携带模板 ID，后端按模板而非模型 ID 识别尺寸区间与专属能力
- **Seedream 官方参数对齐（2026.08 文档）**：5.0 Pro / 4.0 支持提示词优化（标准 / 快速）、5.0 Pro 支持 1K / 1.5K / 2K 像素档与透明背景（i2i 单参考图，自动切 PNG）；5.0 Lite 支持 PNG / JPEG 输出与联网搜索；组图上限 15 张且按「参考图数 + 生成数 ≤ 15」动态收敛；自定义 W/H 同时校验总像素与宽高比 [1/16, 16]；参考图支持 png / jpg / webp / bmp / tiff / gif / heic / heif
...
- **Seedream 5.0 Pro 交互编辑（Draw）**：在参考图上点选 / 框选，自动换算 0–999 归一化坐标并以 `<point>` / `<bbox>` 写入提示词，实现局部替换与元素定位
- **Seedream 5.0 Pro 图层拆分**：单参考图拆分为底图 + 透明 PNG 图层，详情面板展示 `z_index` / 图层名称 / 描述 / 边界框；图层画布支持显隐、拖拽排序并按当前视图合成导出 PNG；元数据存本地 sidecar，不迁移 SQLite 结构
...
| 厂商 | 接入状态 | 图像模型 | 视频模型 |
|------|---------|---------|---------|
| 火山方舟（即梦/豆包） | ✅ | Seedream 5.0 Pro（1.5K / 提示词优化 / 透明背景）/ 5.0 Lite（PNG·JPEG / 联网搜索）/ 4.0（提示词优化）/ 4.5 | Seedance 2.0 / 2.0 Fast / 2.0 Mini / 1.5 Pro / 1.0 Pro / 1.0 Pro Fast |
...
| 参考图收编 | `<数据根目录>\inputs\` |
| 图层拆分元数据 | `<数据根目录>\layers\{history_id}.json`（sidecar，可重建） |
