...
export function DetailPanel({ sources, index, onClose, onNavigate }: DetailPanelProps) {
  ...
  const navBtn = (side: "left" | "right", disabled: boolean, onClick: () => void) => (
    <button
      className={cn(
        "absolute top-1/2 z-10 grid h-12 w-8 -translate-y-1/2 cursor-pointer place-items-center rounded-[100px] border-0 bg-black/30 text-white transition-opacity duration-300 hover:opacity-100",
  ...
  return (
    <div className="fixed inset-0 z-50 bg-card text-foreground animate-[fadeInUp_.15s]">
      <div className="flex h-full w-full bg-card text-foreground">
        {/* 左：图片区 */}
        <div
          ref={containerRef}
          className={cn(
            "group/detail relative flex min-w-0 flex-1 touch-none items-center justify-center overflow-hidden bg-surface px-10 py-16",
        ...
        >
          ...
          <button
            className="absolute top-4 right-4 z-10 grid size-9 cursor-pointer place-items-center rounded-full border-0 bg-card/80 text-muted-foreground transition-colors hover:bg-card hover:text-foreground"
          ...
          {/* 缩放工具条（仅图像） */}
          {source.image && (
            <div className="absolute bottom-5 left-1/2 z-10 flex -translate-x-1/2 items-center gap-0.5 rounded-full border border-border-2 bg-card/90 px-1.5 py-1 text-muted-foreground shadow-[var(--shadow-xs)] backdrop-blur-[6px]">
              <button
                className="grid size-7 cursor-pointer place-items-center rounded-full text-[15px] leading-none transition-colors hover:bg-surface-3 hover:text-foreground"
              ...
              {zoom > 1 && (
                <button
                  className="cursor-pointer rounded-full px-2.5 text-[12px] font-medium text-accent transition-colors hover:bg-accent-hover"
        ...
        <div className="w-px shrink-0 bg-border-2" />
        ...
        <div className="flex w-[456px] shrink-0 flex-col max-[1120px]:w-[400px]">
          <div className="flex-1 overflow-y-auto px-8 pt-5 pb-4">
            ...
            <div className="flex items-center justify-between pb-4">
              <Button
                variant="secondary"
                size="sm"
                className="h-8 gap-1.5 rounded-lg px-2.5 text-[13px] hover:bg-surface-3"
                onClick={download}
              >
                ...
                {source.starred != null && source.onToggleStar && (
                  <Button
                    ...
                    className={cn(
                      ...
                      source.starred ? "text-warn" : "text-muted-foreground",
                    )}
                ...
                <DropdownMenu open={moreOpen} onOpenChange={setMoreOpen}>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      className="rounded-lg text-muted-foreground hover:bg-surface-2"
            ...
            {/* 元信息行 */}
            <div className="flex items-center justify-between border-b border-border-2 pb-5 text-[12px] text-text-3">
            ...
            <div className="mt-5 pb-4">
              <div className="flex items-center justify-between">
                <span className="text-[14px] font-medium text-foreground">{t("gallery.params")}</span>
                <div className="flex items-center gap-1">
                  {paramRows.length > 0 && (
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      className="gap-1 rounded-md px-1.5 text-xs text-text-3 hover:bg-surface-2 hover:text-foreground"
                  ...
                  <Button
                    variant="ghost"
                    size="icon-xs"
                    className="rounded-md text-faint hover:bg-surface-2"
              ...
              <div className={cn("relative", !paramsOpen && "max-h-[150px] overflow-hidden")}>
                <div className="mt-2 flex flex-col gap-4">
                  <div>
                    <div className="flex items-center gap-1">
                      <span className="text-[12px] font-medium text-faint">{t("gallery.prompt")}</span>
                      <Button
                        variant="ghost"
                        size="icon-xs"
                        className="rounded text-faint hover:text-accent"
                  ...
                  {negativePrompt && (
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="text-[12px] font-medium text-faint">{t("prompt.paramNegativePrompt")}</span>
                        <Button
                          variant="ghost"
                          size="icon-xs"
                          className="rounded text-faint hover:text-accent"
                  ...
                  {paramRows.length > 0 && (
                    <div className="rounded-[8px] bg-surface px-3 py-4">
                      {paramRows.map((r) => (
                        <div key={r.label} className="mb-3 flex items-start justify-between gap-4 last:mb-0">
                          <span className="shrink-0 text-xs text-text-2">{r.label}</span>
                          <span className="min-w-0 text-right text-xs break-words whitespace-pre-wrap text-text-2">
                            {r.value}
                          </span>
                        </div>
                      ))}
                  ...
                  {source.image && layerMeta && source.historyId != null && (
                    <Button
                      type="button"
                      variant="ghost"
                      className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-accent-hover px-3 py-2.5 text-xs font-semibold text-accent transition-colors hover:bg-accent-hover"
                ...
                {!paramsOpen && (
                  <button
                    className={cn(
                      ...
                      !hasSelection && "bg-gradient-to-t from-card to-transparent",
                    )}
            ...
            {source.loras && source.loras.length > 0 && (
              <div className="pb-4">
                <div className="pb-3 text-[12px] text-faint">{t("gallery.modelInfo")}</div>
                <div className="flex flex-col gap-2">
                  {source.loras
                    .filter((l) => l.repo.trim())
                    .map((l) => (
                      <div key={l.repo} className="flex items-center gap-2 rounded-[6px] bg-surface p-1.5 pr-2">
                        <span className="grid size-6 shrink-0 place-items-center rounded-full bg-black/50 text-[10px] font-medium text-white">
                          {l.weight}
                        </span>
                        <span className="min-w-0 text-[12px] break-words text-text-3">{l.repo}</span>
                      </div>
                    ))}
                </div>
              </div>
            ...
            <div className="pb-4">
              <div className="pb-3 text-[12px] text-faint">{t("gallery.basicInfo")}</div>
              <div className="flex flex-wrap gap-2">
                <div className="flex h-[28px] items-center gap-2 rounded-[6px] bg-surface p-[2px] pr-2">
                  ...
                  <span className="text-[12px] text-text-3">{source.model}</span>
                </div>
                {source.size && source.size !== source.ratio && <div className="flex h-[28px] items-center rounded-[6px] bg-surface px-2 text-[12px] text-text-3">{source.size}</div>}
                {source.ratio && <div className="flex h-[28px] items-center rounded-[6px] bg-surface px-2 text-[12px] text-text-3">{source.ratio}</div>}
              </div>
            </div>
          </div>
          ...
          <div className="shrink-0 px-8 pb-6">
            <div className="rounded-lg bg-surface px-1 pb-4">
              <h1 className="px-3 pt-4 text-[14px] font-medium text-faint">
...
<div className="mt-2 flex gap-2 px-2 pb-1">
                {source.image && source.onImageToVideo && (
                  <Button
                    variant="ghost"
                    className="h-11 min-w-0 flex-1 rounded-lg p-3 text-[14px] hover:bg-surface-2"
                ...
                {source.image && source.onImageToImage && (
                  <Button
                    variant="ghost"
                    className="h-11 min-w-0 flex-1 rounded-lg p-3 text-[14px] text-foreground hover:bg-surface-2"
